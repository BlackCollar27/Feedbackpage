...
2. **Set Environment Variables**
   - In Supabase dashboard, add `STRIPE_SECRET_KEY` to your Edge Function secrets
   - In your frontend `.env`, add `VITE_STRIPE_PUBLIC_KEY`

3. **Update Server Code**
   - Uncomment the Stripe SDK code in `/supabase/functions/server/index.tsx`
   - Update the price IDs in the `create-checkout-session` endpoint

4. **Configure Webhook**
   - Set up a webhook endpoint in Stripe Dashboard
   - Point it to: `https://your-project.supabase.co/functions/v1/make-server-6b2adf01/stripe-webhook`
   - Add the webhook secret to your environment variables

### Email Configuration (Optional)

By default, Supabase auto-confirms user emails. To enable email verification:

1. Configure an email provider in Supabase Dashboard
2. Update the signup endpoint to remove `email_confirm: true`
3. Users will receive verification emails

## Data Persistence

All data is stored in Supabase using a key-value store:
- Business profiles

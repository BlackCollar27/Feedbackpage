...
3. **Backend functions** (`/supabase/functions/server/`)

### Files to Update for Rails Backend:

#### 1. **Authentication Context** (`/src/app/contexts/AuthContext.tsx`)
Replace Supabase Auth with Rails session-based auth:
```typescript
// Current: Uses Supabase auth
// Replace with: Rails session/JWT authentication
```

#### 2. **API Client** (`/src/app/api/client.ts`)
Update API base URL to point to Rails backend:
```typescript
// Current: const API_BASE = `https://${projectId}.supabase.co/functions/v1/make-server-6b2adf01`
// Replace with: const API_BASE = process.env.VITE_API_BASE_URL || 'https://api.feedback-page.com'
```

#### 3. **Onboarding Component** (`/src/app/components/Onboarding.tsx`)
Remove Supabase client imports and replace with Rails API calls.
...
1. **Remove Supabase Dependencies**: 
   - The frontend currently has Supabase auth/database code
   - All of this needs to be replaced with Rails API calls
   - Delete `/supabase` folder after migration
